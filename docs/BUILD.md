...
| 대상 | 결과 |
|---|---|
| `main` (실행파일) | ✅ MD5 `3ef0843d…` — 릴리즈 v1.4와 **바이트 단위 동일** |
| RDB 본문 (CHK 13,433개) | ✅ **전부 동일**, 다른 파일 0개 |
| RDB 파일 MD5 | ⚠ 배포본과 다름 — **정상입니다** |

RDB의 MD5가 달라지는 이유는, 커진 파일을 어느 빈 섹터로 옮겼는지가 빌드 이력에 따라
달라지기 때문입니다. 담긴 내용은 같습니다. 그래서 RDB는 MD5가 아니라 **본문 단위로** 비교해야 합니다.

```bash
python tools/VERIFY_RDB_BODIES.py <내가_빌드한>/repack_out <비교대상>/repack_out
```

> 이 검증 과정에서 실제 버그가 하나 잡혔습니다. 파일을 제자리에 쓸 수 있는 한계를
> *현재* 파일 크기로 계산하는 바람에, 재배치로 파일이 커지면 마지막 파일의 여유를 과대
> 계산해 **재배치해 둔 데이터를 덮어썼습니다**(해당 CHK가 zlib 해제 실패). 상한을 원본
> 파일 끝으로 고정해 수정했습니다. 배포본은 이미 여유 공간이 많은 상태에서 작업해
> 드러나지 않았고, **원본에서 처음부터 빌드할 때만** 나타나던 문제입니다.
