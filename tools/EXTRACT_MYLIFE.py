...
for sc in scenes:
    ...
    if score == 0: continue               # 마이라이프 고유 키워드 필수
    if not is_dialogue_scene(sc): continue  # UI/목록 배제
    in_va = True
...
json.dump(doc, open('마이라이프_대사.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
print(f"씬 {len(out)} / 문장 {n_sent} (2조각+ {n_frag_sent}) / 엔트리매핑 {mapped}/{tot_frag}")
print(f"→ 마이라이프_대사.json ({os.path.getsize('마이라이프_대사.json')/1024:.0f}KB)")
# 표본
for sc in out[:2]:
